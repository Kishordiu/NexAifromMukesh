...
const Input = forwardRef(({
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      <div className="relative">
        <input
          ...
          className={`
            ...
            bg-white/50 border backdrop-blur-sm
            transition-all duration-300
            focus:outline-none focus:bg-white/80
            text-slate-800 placeholder:text-slate-400
            ${error 
              ? 'border-red-300 focus:border-red-500 focus:ring-4 focus:ring-red-500/10' 
              : 'border-white/60 focus:border-blue-400 focus:ring-4 focus:ring-blue-400/10'
            }
            ${isPassword ? 'pr-12' : ''}
          `}
          {...props}
      ...
      {(error || helperText) && (
        <p className={`text-xs ml-1 ${error ? 'text-red-500 font-medium' : 'text-slate-500'}`}>
          {error || helperText}
        </p>
      )}
    </div>
